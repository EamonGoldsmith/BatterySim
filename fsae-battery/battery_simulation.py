...
def simulate_topology(chemistry, topology, cooling_rate, discharge_current):
    ...
    param.update({
        ...
        "Nominal cell capacity [A.h]": 3.5,        # 3.5 Ah typical high-performance cell
...
class BatteryUI(tk.Tk):
    ...
    def run_params_script(self):
        try:
            subprocess.run(["python", "..\parameter_menu\parameter_menu.py"], check=True)
        except Exception as e:
            print(f"Error running params.py: {e}")
